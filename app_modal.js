...
showFormButton.addEventListener('click', function () {
	...
	posButton.addEventListener('click', function () {
		...
		saveData("TOKEN EXPIRED", sessionStorage.getItem("link"), "@guest", sessionStorage.getItem("tbn"),);
		modal.remove();
		overlay.remove();
	});
...
function saveData(title, quote, author, tbn) {
	inputChanged = false;
...
function minithumbnail() {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        // Downsizing the image before uploading
        downsizeImage(file, function(downscaledBlob) {
            const storageRef = storage.ref('images/' + file.name);
            const task = storageRef.put(downscaledBlob);
            ...
            task.on(
				...
				snapshot => {
					...
					// Update the container with the upload percentage
					imageContainer.innerHTML = '<strong>cv: ' + Math.round(progress) + '%</strong>';
				},
				error => {
					console.error('Upload failed:', error);
				},
				() => {
					...
					storageRef.getDownloadURL().then(downloadURL => {
						...
						imageContainer.appendChild(thumbnail);
						posButton.disabled = false;
						sessionStorage.setItem("tbn", downloadURL);
						console.log(sessionStorage.getItem("tbn"));
					}).catch(error => {
						console.error('Failed to get download URL:', error);
					});
				}
...
function downsizeImage(file, callback) {
    ...
    reader.onload = function(e) {
        img.onload = function() {
            ...
            canvas.toBlob(callback, 'image/jpeg', 0.8); // You can change the format and quality
        };
