...
showFormButton.addEventListener('click', function () {
	...
	let imgButton = "<input type='file' id='img-button' onchange='handleImage()' multiple accept='image/*'>";
	...
	posButton.addEventListener('click', function () {
		if (sessionStorage.getItem("link") === "?") {
			if (saveData("ehst", "?", "@username", "?", Textarea.value)) {
				modal.remove();
				overlay.remove();
			}
		} else {
			if (saveData("TOKEN EXPIRED", sessionStorage.getItem("link"), "@guest", sessionStorage.getItem("base64"), Textarea.value)) {
				modal.remove();
				overlay.remove();
			}
